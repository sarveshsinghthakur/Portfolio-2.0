    ...
    <section id="leetcode" className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <p className="max-w-2xl text-sm leading-7 text-muted-foreground md:text-base">
          This section uses your public LeetCode profile snapshot so it feels closer to the dashboard you shared while matching the new minimalist design.
        </p>
      </div>

      <div className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
        <article className="rounded-[28px] border border-border bg-background p-6 shadow-sm">
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-2">
              <div>
                <h3 className="text-2xl font-semibold text-foreground">{leetcodeSnapshot.name}</h3>
                <p className="text-sm text-muted-foreground">@{leetcodeSnapshot.username}</p>
              </div>
              <p className="text-lg text-foreground/80">Rank {leetcodeSnapshot.ranking}</p>
              <p className="text-sm leading-7 text-muted-foreground">{leetcodeSnapshot.about}</p>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            <div className="rounded-[22px] border border-border bg-secondary p-4">
              ...
              <p className="mt-2 text-foreground">{leetcodeSnapshot.country}</p>
            </div>
            <div className="rounded-[22px] border border-border bg-secondary p-4">
              <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <p className="mt-2 text-foreground">{leetcodeSnapshot.school}</p>
            </div>
          </div>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {leetcodeFocusTags.map((tag) => (
              <span
                key={tag}
                className="rounded-full border border-border bg-primary/10 px-3 py-2 text-xs uppercase tracking-[0.18em] text-primary"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="grid gap-5">
          <div className="grid gap-5 lg:grid-cols-[0.78fr_1.22fr]">
            <article className="rounded-[28px] border border-border bg-background p-6 shadow-sm">
              <div className="mx-auto flex max-w-[210px] flex-col items-center">
                <div
                  ...
                  style={{
                    background: `conic-gradient(hsl(var(--primary)) 0deg ${solvedRatioDegrees}deg, rgba(0,0,0,0.08) ${solvedRatioDegrees}deg 360deg)`,
                  }}
                >
            ...
            <article className="grid gap-4 rounded-[28px] border border-border bg-background p-6 shadow-sm sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
              {[
                ...
                <div
                  key={item.label}
                  className="rounded-[22px] border border-border bg-secondary p-5"
                >
          ...
          <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
            <article className="rounded-[28px] border border-border bg-background p-6 shadow-sm">
              ...
              <div className="mt-5 rounded-[22px] border border-border bg-secondary p-4">
                <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
                  Most Recent Badge
                </p>
              ...
              <p className="mt-4 text-xs uppercase tracking-[0.18em] text-muted-foreground/60">
            ...
            <article className="rounded-[28px] border border-border bg-background p-6 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-lg font-medium text-foreground">Most solved by language</p>
                </div>
              </div>
          ...
          <article className="rounded-[28px] border border-border bg-background p-6 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-medium text-foreground">Problem categories solved the most</p>
              </div>
            </div>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
              {leetcodeSnapshot.strongTopics.map((topic) => (
                ...
                >
                  ...
                  <p className="mt-3 text-2xl font-semibold text-foreground">{topic.solved}</p>
                </div>
              ))}
            </div>
          </article>
