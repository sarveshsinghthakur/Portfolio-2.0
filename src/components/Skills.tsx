    ...
    <section id="skills" className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <p className="max-w-2xl text-sm leading-7 text-white/55 md:text-base">
          ...
          presented in a darker product-focused layout inspired by your reference site.
        </p>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        {expertiseItems.map((item, index) => {
          ...
          return (
            <article
              key={item.title}
              className="group rounded-[28px] border border-white/6 bg-[#111111] p-6 shadow-[0_18px_60px_rgba(0,0,0,0.28)] transition hover:-translate-y-1 hover:border-primary/35 hover:shadow-[0_24px_80px_rgba(66,255,145,0.12)]"
            >
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {item.technologies.map((tech) => (
                  <span
                    key={tech}
                    className="rounded-full border border-white/8 bg-black/30 px-3 py-2 text-xs uppercase tracking-[0.18em] text-white/62"
                  >
                    {tech}
                  </span>
                ))}
